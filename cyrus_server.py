            ...
            if decision is None:
                if _is_answer_request(text):
                    ...
                    if len(words) > SUMMARY_WORD_THRESHOLD:
                        resp = " ".join(words[:SUMMARY_WORD_THRESHOLD]) + ". See the chat for more."
                    decision = {"action": "answer", "spoken": resp, "message": "", "command": {}}
                else:
                    decision = {"action": "forward", "message": text + VOICE_HINT,
                                "spoken": "", "command": {}}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Cyrus Remote Brain (WebSocket server)")
